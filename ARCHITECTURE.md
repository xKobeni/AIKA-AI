...
│   │   └── models.py                        # ORM models (Memory, Conversation)
│   ├── repositories/
│   │   ├── __init__.py
...
1. **Chat flow** — The most common path. The intent classifier determines the user wants to chat. The context manager gathers recent conversation history and relevant memories. The chat handler sends them to Ollama and returns the response. Memory extraction runs in the background.
...
### Brain (`src/brain/`)
- **`AikaBrain`** — Top-level orchestrator. Initializes all services, repositories, handlers, and tools. Exposes `process(user_message)` which runs the entire pipeline.
- **`DecisionEngine`** — Uses the intent classifier to decide whether the user wants to chat, retrieve memories, use a tool, or execute a plan.
- **`Router`** — Routes the decision to the appropriate handler.
- **`ContextManager`** — Builds context windows by merging conversation history, relevant memories, and profile data.
...
- **`models.py`** — Defines `Memory` (id, type, content, embedding, importance, category, timestamps) and `Conversation` (id, role, content, timestamp) via SQLAlchemy ORM.
...
- **`ChatHandler`** — Generates responses using the LLM, augmented with context.
- **`MemoryHandler`** — Responds to memory queries and manages memory retrieval.
- **`MemoryExtractor`** — Asynchronously extracts and stores memories from conversations.
